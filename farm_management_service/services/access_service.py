...
class AccessService:
    ...
    async def grant_access(
        self, farm_id: str, access_data: FarmAccessCreate, grantor_id: str
    ):
        ...
        if owner_id != grantor_id:
            raise HTTPException(
                status_code=403, detail="Only the farm owner can grant access"
            )

        # 2. Check if access already exists
        existing_access = await self.access_repo.get_access_entry(
            farm_id, access_data.user_id
        )

        if existing_access:
            # Update existing access
            return await self.access_repo.update_access(
                existing_access, access_data.access_level
            )

        # 3. Create new access entry
        return await self.access_repo.create_access(
            farm_id, access_data.user_id, access_data.access_level
        )
    ...
    async def revoke_access(self, farm_id: str, target_user_id: str, requestor_id: str):
        ...
        if owner_id != requestor_id:
            raise HTTPException(
                status_code=403, detail="Only the farm owner can revoke access"
            )
    ...
    async def list_access(self, farm_id: str, requestor_id: str):
        ...
        if not has_perm:
            raise HTTPException(
                status_code=403, detail="Not enough permissions to view access list"
            )

        return await self.access_repo.get_all_access_for_farm(farm_id)

    async def has_access(
        self, farm_id: str, user_id: str, required_level: AccessLevel
    ) -> bool:
        # 1. Check if Owner
        owner_id = await self.access_repo.get_farm_owner(farm_id)
