...
async def test_get_rule_by_id(client: AsyncClient):
    ...
    all_rules_response = await client.get(
        "/api/rule-service/rules/all?farm_id=farm-id-test"
    )
...
async def test_update_rule(client: AsyncClient):
    ...
    all_rules_response = await client.get(
        "/api/rule-service/rules/all?farm_id=farm-update"
    )
    rule_id = all_rules_response.json()["items"][0]["rule_id"]
    ...
    response = await client.put(
        f"/api/rule-service/rules/rule/{rule_id}", json=update_payload
    )
    assert response.status_code == 200
...
async def test_delete_rule(client: AsyncClient):
    ...
    all_rules_response = await client.get(
        "/api/rule-service/rules/all?farm_id=farm-delete"
    )
    rule_id = all_rules_response.json()["items"][0]["rule_id"]
