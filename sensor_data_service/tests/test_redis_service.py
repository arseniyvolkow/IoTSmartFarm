...
async def test_redis_connect_success(redis_service):
    with patch("sensor_data_service.services.redis_service.redis_client.Redis") as mock_redis_cls:
...
async def test_redis_device_twin(redis_service):
    ...
    redis_service.client.get.return_value = orjson.dumps({"desired": {"temp": 20}, "reported": {}}).decode('utf-8')
    twin = await redis_service.get_device_twin("device_1")
