...
async def test_stream_consumer_listen(mock_redis_service):
        ...
        [
            (
                ...
                [
                    (
                        "1-0",
                        {"data": json.dumps({"sensor_id": "sensor_1", "value": 25.5})},
                    ),
                    (
                        "2-0",
                        {
                            "data": json.dumps(
                                {"sensor_id": "sensor_1", "value": "not_numeric"}
                            )
                        },
                    ),
                    ("3-0", {"data": "not_json"}),
    ...
    evaluator.evaluate_rules_for_sensor.assert_called_once_with(
        "sensor_1", 25.5, mock_session
    )
