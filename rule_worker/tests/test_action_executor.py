...
async def test_execute_device_control(mock_redis_service):
    ...
    result = await executor.execute({
        "action_type": RuleActionType.CONTROL_DEVICE.value,
        "action_payload": payload
    })
    
    assert result is True
    mock_redis_service.publish.assert_called_once_with("actuator_commands", json.dumps(payload))
...
async def test_execute_log_event(mock_redis_service):
    ...
    result = await executor.execute({
        "action_type": RuleActionType.LOG_EVENT.value,
        "action_payload": payload
    })
