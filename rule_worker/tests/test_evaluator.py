...
async def test_is_rule_on_cooldown(sample_rule):
    ...
    evaluator = RuleEvaluator(
        MagicMock(spec=ActionExecutor), MagicMock(spec=RuleContextBuilder)
    )
...
async def test_evaluate_single_rule_match(
    sample_rule, mock_db_session, mock_action_executor, context_builder
):
...
async def test_evaluate_single_rule_no_match(
    sample_rule, mock_db_session, mock_action_executor, context_builder
):
    evaluator = RuleEvaluator(mock_action_executor, context_builder)
    ...
    with patch.object(
        context_builder, "build", return_value={"value": 5, "sensor_id": "sensor_1"}
    ):
        result = await evaluator.evaluate_single_rule(sample_rule, mock_db_session)
...
async def test_evaluate_all_rules(
    sample_rule, mock_db_session, mock_action_executor, context_builder
):
    evaluator = RuleEvaluator(mock_action_executor, context_builder)
...
async def test_evaluate_rules_for_sensor(
    sample_rule, mock_db_session, mock_action_executor, context_builder
):
    evaluator = RuleEvaluator(mock_action_executor, context_builder)
