                ...
                for stream, msg_list in messages:
                    for message_id, msg_data in msg_list:
                        try:
                            ...
                            if raw_payload:
                                logger.info(
                                    f"📩 Received sensor update stream msg: {raw_payload}"
                                )
                                ...
                                if sensor_id is not None and value is not None:
                                    try:
                                        ...
                                        async with get_db() as db_session:
                                            await self.evaluator.evaluate_rules_for_sensor(
                                                sensor_id, val_float, db_session
                                            )
                                    except ValueError:
                                        logger.warning(
                                            f"Received non-numeric value for sensor {sensor_id}: {value}"
                                        )

                            # Acknowledge the message so it's removed from PEL
                            await self.redis_service.ack_message(
                                stream_name, group_name, message_id
                            )

                        except json.JSONDecodeError:
                            logger.error(
                                f"Failed to decode sensor update message: {msg_data}"
                            )
