...
class RuleEvaluator:
    ...
    def __init__(
        self, action_executor: ActionExecutor, context_builder: RuleContextBuilder
    ):
    ...
    def _is_rule_on_cooldown(self, rule: Rules) -> bool:
        ...
        if is_on_cooldown:
            logger.debug(
                f"Rule '{rule.rule_name}' (ID: {rule.rule_id}) is on cooldown. Skipping."
            )
        return is_on_cooldown

    async def _execute_matched_rule_actions(
        self, rule: Rules, context: dict[str, Any], db: AsyncSession
    ):
        ...
        for action in sorted_actions:
            ...
            try:
                ...
                if not success:
                    logger.warning(
                        f"⚠️ Action {action.action_id} failed for rule '{rule.rule_name}'."
                    )
        ...
        try:
            ...
            stmt = (
                update(Rules)
                .where(Rules.rule_id == rule.rule_id)
                .values(last_triggered_at=now_dt)
            )
            ...
            logger.error(
                f"Failed to update last_triggered_at for rule {rule.rule_id}: {e}"
            )
            await db.rollback()
    ...
    async def evaluate_single_rule(
        ...
        try:
            context = await self.context_builder.build(
                rule, triggered_value=triggered_value
            )
            if context is None:
                return False
    ...
    async def evaluate_all_rules(
        self, db_session: AsyncSession, trigger_type: RuleTriggerType | None = None
    ):
        ...
        try:
            ...
            logger.info(
                f"📋 Evaluating {len(rules)} active rules (type: {trigger_type or 'all'})"
            )
    ...
    async def evaluate_rules_for_sensor(
        ...
        try:
            ...
            logger.info(
                f"🎯 Evaluating {len(rules)} rules for updated sensor: {sensor_id} (value: {value})"
            )

            tasks = [
                self.evaluate_single_rule(rule, db_session, triggered_value=value)
                for rule in rules
            ]
            await asyncio.gather(*tasks, return_exceptions=True)
